define("calendarheatmap", ["jquery", "moment", "d3"], function($, moment, d3) {
  var calendarHeatmap = function() {
    ...
    function chart(nowDate, yearAgoDate) {
      ...
      d3.select("body")
        .selectAll("div.day-cell-tooltip")
        .remove(); // remove existing tooltips
      ...
      function drawChart() {
        ...
        dayRects
          ...
          .attr("y", function(d, i) {
            return (
              MONTH_LABEL_PADDING +
              d.getDay() * (SQUARE_LENGTH + SQUARE_PADDING)
            );
          });
        ...
        var monthLabels = svg
          ...
          .attr("x", function(d, i) {
            ...
            for (
              var j = 0;
              j < dateRange.length && d.getTime() > dateRange[j].getTime();
              j++
            ) {
              if (dateRange[j].getDay() === 0) {
                // sunday
                matchIndex++;
              }
            }
      ...
      function tooltipHTMLForDate(d) {
        ...
        html = html.replace(
          /\%unit\%/g,
          count === 1 ? tooltipUnit : tooltipUnitPlural
        );
